...
namespace UserMasterAPI.Controllers
{
    ...
    public class UserMasterController : ControllerBase
    {
        ...
        public APIResponse ActivateUser([FromBody] ActivateUserRequest request)
        {
            string result = string.Empty;
            try
            {
                var _emp = context.TblUser.FirstOrDefault(o => o.Userid == request.Userid);
                if (_emp != null)
                {
                    _emp.Role = request.Role;
                    _emp.IsActive = request.IsActive;
                    context.SaveChanges();
                    result = "pass";
                }
            }
